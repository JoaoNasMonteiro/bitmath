...
local function evaluate_current_line()
	...
	api.nvim_buf_clear_namespace(state.bufnr, state.ns_id, row, row + 1)
...
local function setup_buffer()
	if not state.bufnr or not api.nvim_buf_is_valid(state.bufnr) then
		...
		vim.keymap.set("n", "<CR>", evaluate_current_line, {
			buffer = state.bufnr,
			desc = "Evaluate line as BitMath",
		})

		vim.keymap.set("i", "<CR>", function()
			evaluate_current_line()
		end, {
			buffer = state.bufnr,
			desc = "Evaluate line as BitMath (Insert Mode)",
		})
	end
	return state.bufnr
end
